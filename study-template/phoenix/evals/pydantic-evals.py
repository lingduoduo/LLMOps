...
class MatchesExpectedOutput(Evaluator[str, str]):
    def evaluate(self, ctx: EvaluatorContext[str, str]) -> float:
        ok = (ctx.expected_output or "").strip() == (ctx.output or "").strip()
...
class FuzzyMatchesOutput(Evaluator[str, str]):
    def evaluate(self, ctx: EvaluatorContext[str, str]) -> float:
        ...
        a = (ctx.expected_output or "").strip().casefold()
        b = (ctx.output or "").strip().casefold()
        return 1.0 if SequenceMatcher(None, a, b).ratio() >= 0.85 else 0.0
...
results = EvaluationReportAdapter.dump_python(report)
print(results)
